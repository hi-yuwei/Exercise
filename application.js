    ...
    express.init = function () {
        ...
        $.fn.sendCode = function (url, dataTemp, mobinput, _useImg) {
            var waitSec = 120, //等待时间
                _this = this,
                useImg = _useImg ? _useImg : true;  //默认不使用图片验证码
            this.click(function () {
                            ...
                            $(".imgVerifyBtn").eq(0).click(function () {
                                if (($("#captcha").val().length) == 4) {
                                    ...
                                    if (sendNode.hasClass("cookie-auto-change")) {
                                        ...
                                        data = (typeCookie == "voice" || typeCookie == undefined) ? {
                                            mobile: mo,
                                            captcha: ca,
                                            voice: false
                                        } : {mobile: mo, captcha: ca, voice: true}
                                    }
                                    docheck(function () {
                                        layer.close(imgVerifyView);
                ...
                function docheck(_callback) {
                    $.ajax({
                        ...
                        success: validMethods.createAjaxCallback(function (status, form, data) {
                            if (data && !data.success) {
                                if ((data[0].field == "verifyNo" && data[0].message) || (data[0].field == "outPassword" && data[0].message == "交易密码错误，请重新输入。")) {
                                    layer.close(imgVerifyView);
                                    return false;
                                }
                            }
...
//倒计时修改结束
    ...
    $(function () {
        ...
        $(".ajaxAlert").click(function () {
            $(this).blur();
            layer.alertUrl($(this).attr("href") + "?_iframe=true&_redirect=" + window.location.href, $(this).attr("title"));
            return false;
        });

        /**
         * 有ajaxGlide类的元素设置单击事件平滑过渡效果 20160129
         */
        $(".goto").attr("href", "javascript:;");
        $(".goto").click(function () {
            $('html, body').animate({
